      ...
      <el-dialog title="贷款审批-查看详情" :visible="dialogVisible" @close="dialogVisible=false" width="30%">
          <div class="form-box" v-if="dialogVisible">
              ...
              <div class="btn clear-fix">
                  <div>
                      <el-button types="primary" @click="dialogVisible=!dialogVisible">取消</el-button>
                  </div>
              </div>
          </div>
      </el-dialog>
  ...
  export default {
      ...
      methods:{
          ...
          convertBirthday(date){
              ...
              return `${d2.getFullYear()}-${convertTime(d2.getMonth() + 1)}-${convertTime(d2.getDate())}`
          },
